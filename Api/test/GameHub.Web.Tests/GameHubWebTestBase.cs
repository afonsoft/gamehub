...
{
    public abstract class GameHubWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        ...
        {
            using (var context = IocManager.Resolve<GameHubDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected async Task UsingDbContextAsync(Func<GameHubDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<GameHubDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<GameHubDbContext, Task<T>> func)
        {
            T result;
